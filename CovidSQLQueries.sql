-- SCRIPTS USED FOR VISUALIZATION RESULTS
...
-- Most Recent Mortality rate by country
...
-- Global Rolling Crude Mortality Rate
WITH cte_rolling_death_count as (
...
INNER JOIN DAPortf2..CovidPopData c
ON (a.iso_code = c.iso_code and a.date = c.date)
WHERE a.location NOT IN ('European Union')
)
SELECT date, ROUND(AVG(dateTotalDeaths/totalGlobalPop),9) FROM cte_rolling_death_count group by date order by date

-- Global Rolling Crude Infection Rate

WITH cte_rolling_infection_count as (
SELECT a.location, a.date,  new_cases,
	SUM(a.new_cases) OVER (PARTITION BY a.location order by a.location,a.date) toDateTotalCases
from DAPortf2..CovidDeathsAndCases a
INNER JOIN DAPortf2..CovidPopData c
ON (a.iso_code = c.iso_code and a.date = c.date)
WHERE a.location IN ('Africa', 'North America', 'South America', 'Oceania','Asia','Europe')
)
SELECT * FROM cte_rolling_infection_count order by location, date


-- Unused Scripts for Visualization
-- Select all script
SELECT TOP (5000) *
  FROM DAPortf2..CovidDeathsAndCases a
...
-- National Rolling Count of Infected
-- Sum of all nation's new cases/sum of all nation's deaths
